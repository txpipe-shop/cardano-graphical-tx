<script lang="ts">
    ...
    import { DatumType, type Metadata } from "@/types/utxo-model";
  ...
  function parseValues(value: Record<string, bigint>): { policy: string, values: Record<string, bigint> }[] {
...
<Ccollapsible title="MINTS AND BURNS">
  ...
  {#each parseValues(tx.mint) as value}
    <Ccollapsible title={`Policy: ${value.policy}`}>
      {#each Object.entries(value.values) as [assetName, amount]}
        <Box title={`Asset Name: Amount`} content={`${assetName}: ${amount}`} bg={amount > 0 ? "bg-green-900" : "bg-red-900"}/>
      {/each}
    </Ccollapsible>
  {/each}
</Ccollapsible>
