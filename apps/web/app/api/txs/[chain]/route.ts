...
export async function GET(req: Request, { params }: { params: { chain: string } }) {
  ...
  try {
    ...
    const result = await provider.getTxs({ limit: EXPLORER_PAGE_SIZE, offset, query: undefined });

    return new Response(
      JSONBIG.stringify({ data: result.data, hasMore: result.data.length === Number(EXPLORER_PAGE_SIZE) }),
